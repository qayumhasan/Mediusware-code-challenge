...
export default {
    ...
    methods: {
        ...
        updateProduct() {
            ...
            axios.put(`/api/product/${this.product.id}`, product).then(response => {
    ...
    mounted() {
        ...
        for (const key in this.update_varients){
             let variant_option ={};
            variant_option.option = key;
            let tags = [];
            this.update_varients[key].forEach(item=>{
                tags.push(item.variant);
            })
            variant_option.tags = tags;

            this.product_variant.push(variant_option)
            
        }


        this.product_price.map(item=>{
            this.product_variant_prices.push({
                    title: item.Variants,
                    price: item.price,
                    stock: item.stock
